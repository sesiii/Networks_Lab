...
//R NO: 22CS10020
//Assignment 5
//client:program to simulate a client that requests a task repeatedly but does not complete it, 
...
int main() {
    ...
    printf("Connecting to server, requesting tasks repeatedly...\n");
    char buffer[MAX_BUFFER];
    
    while (1) {
        strcpy(buffer, "GET_TASK");
        write(client_socket, buffer, strlen(buffer));
        printf("Client %d: Requested task\n", getpid());
