...
//         shm->sockets[i].rwnd.prev_rwnd = WINDOW_SIZE;
...
//     float drop_prob = args->drop_prob;
//     printf("[thread_r] Starting with shmid=%d, drop_prob=%.2f...\n", shmid, drop_prob);
//     shm = attach_shared_memory(shmid);
...
//                         printf("[thread_r] Dropped message seq=%d due to drop_prob=%.2f\n", msg.seq_num, drop_prob);
//                         continue;
//                     }
//                     int dest_sockfd = (ntohs(sender.sin_port) == PORT_USER1) ? 1 : 0;
//                     int sender_sockfd = (ntohs(sender.sin_port) == PORT_USER1) ? 0 : 1;

//                     if (msg.type == MSG_TYPE_DATA) {
//                         int expected = shm->sockets[dest_sockfd].rwnd.base;
//                         if (msg.seq_num < expected) {
//                             printf("[thread_r] Dropped duplicate for socket %d: seq=%d (expected %d)\n", dest_sockfd, msg.seq_num, expected);
//                         } else if (shm->sockets[dest_sockfd].rwnd.rwnd > 0 && msg.seq_num >= expected && msg.seq_num < expected + WINDOW_SIZE) {
...
//                                     printf("[thread_r] Received for socket %d: seq=%d, %s\n", dest_sockfd, shm->sockets[dest_sockfd].rwnd.messages[0].seq_num, shm->sockets[dest_sockfd].rwnd.messages[0].payload);
//                                 }
...
//                             }
//                         } else {
//                             printf("[thread_r] Dropped for socket %d: seq=%d (expected %d, rwnd=%d)\n", dest_sockfd, msg.seq_num, expected, shm->sockets[dest_sockfd].rwnd.rwnd);
//                         }

//                         KTP_Message ack;
//                         ack.type = MSG_TYPE_ACK;
//                         ack.seq_num = (shm->sockets[dest_sockfd].rwnd.base - 1) % 256;
...
//                         if (shm->sockets[dest_sockfd].rwnd.prev_rwnd == 0 && shm->sockets[dest_sockfd].rwnd.rwnd > 0) {
//                             sendto(socks[i], &ack, sizeof(KTP_Message), 0, (struct sockaddr*)&sender, sizeof(struct sockaddr_in));
//                             printf("[thread_r] Sent duplicate ACK for socket %d: seq=%d, rwnd=%s (nospace cleared)\n", dest_sockfd, ack.seq_num, ack.payload);
//                         }
//                         shm->sockets[dest_sockfd].rwnd.prev_rwnd = shm->sockets[dest_sockfd].rwnd.rwnd;
...
//         // Send queued messages (temporary until thread_s)
//         for (int i = 0; i < MAX_SOCKETS; i++) {
//             if (!shm->sockets[i].is_free && shm->sockets[i].send_count > 0 && shm->sockets[i].swnd.size < WINDOW_SIZE) {
//                 KTP_Message msg = shm->sockets[i].send_buffer[0];
//                 sendto(socks[i], &msg, sizeof(KTP_Message), 0, (struct sockaddr*)&shm->sockets[i].dest_addr, sizeof(struct sockaddr_in));
...
//                 printf("[thread_r] Sent from socket %d: seq=%d, %s\n", i, msg.seq_num, msg.payload);
//             }
//         }
//     }
...
void *thread_r(void *arg) {
    ...
    int socks[MAX_SOCKETS];
    int ports[MAX_SOCKETS] = {PORT_USER1, PORT_USER2};
    for (int i = 0; i < MAX_SOCKETS; i++) {
        socks[i] = socket(AF_INET, SOCK_DGRAM, 0);
        if (socks[i] < 0) {
            perror("[thread_r] Socket creation failed");
        ...
        if (setsockopt(socks[i], SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
            perror("[thread_r] setsockopt SO_REUSEADDR failed");
            for (int j = 0; j <= i; j++) close(socks[j]);
            detach_shared_memory(shm);
            return NULL;
        }
        ...
        inet_pton(AF_INET, "127.0.0.1", &addr.sin_addr);
        if (bind(socks[i], (struct sockaddr*)&addr, sizeof(addr)) < 0) {
            fprintf(stderr, "[thread_r] Bind failed for port %d: %s\n", ports[i], strerror(errno));
            for (int j = 0; j <= i; j++) close(socks[j]);
            detach_shared_memory(shm);
            return NULL;
        }
    ...
    for (int i = 0; i < MAX_SOCKETS; i++) {
        if (socks[i] >= 0) {
            close(socks[i]);
            printf("[thread_r] Closed socket %d (fd=%d)\n", i, socks[i]);
        }
    }
    detach_shared_memory(shm);
    return NULL;
