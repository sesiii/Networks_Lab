...
enum { STATE_INIT = 0, STATE_HELO = 1, STATE_MAIL_FROM = 2, STATE_RCPT_TO = 3, STATE_DATA = 4 };
...
int main(int argc, char *argv[]) {
    ...
    fflush(stdout); // Flush output to ensure it’s written to server_log.txt
...
void process_command(int client_fd, char *buffer, char *sender, char *recipient, int *in_data, char *email_body, int *state) {
    ...
    if (strncmp(buffer, "HELO", 4) == 0) {
        ...
        *state = STATE_DATA;
        ...
        sprintf(response, "354 Start mail input; end with <CRLF>.<CRLF>\n");
        write(client_fd, response, strlen(response));
    }
    else if (*in_data) {
        if (strcmp(buffer, ".\n") == 0) {
            *in_data = 0;
            ...
            sem_post(sem);
            write(client_fd, response, strlen(response));
            *state = STATE_HELO; // Reset to allow new email
        } else {
            printf("%s", buffer);
