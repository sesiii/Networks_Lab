...
int main() {
    ...
    static uint16_t client_trans_id = 0; // Unique client transaction ID counter
    ...
    while (1) {
        ...
        struct iphdr iph = { .version = 4, .ihl = 5, .ttl = 64, .protocol = PROTOCOL_NUM };
        uint16_t trans_id = client_trans_id++; // Incrementing client-specific ID
        struct cldp_header cldp = { 0x02, 1, htons(trans_id), 0 };
        uint8_t payload = (uint8_t)query_type;
        iph.id = htons(rand() % 65535);
        iph.tot_len = htons(sizeof(iph) + sizeof(cldp) + 1);
                ...
                if (len > 0) {
                    ...
                    if (rx_iph->protocol == PROTOCOL_NUM) {
                        ...
                        if (rx_cldp->msg_type == 0x03 && ntohs(rx_cldp->trans_id) == trans_id) {
                            ...
                            response_received = 1;
                            break;
                        } else if (rx_cldp->msg_type == 0x01) {
                            printf("Received HELLO from %s (Trans ID: %u)\n",
                                   inet_ntoa(*(struct in_addr *)&rx_iph->saddr), ntohs(rx_cldp->trans_id));
                        }
                    }
                }
            }
        }
        if (!response_received) printf("No response received within timeout.\n");
    }
